...
try {
    ...
    console.error('[@cw-suite/api-events] Unable to read package.json:', error.message);
    process.exit(1);
}
...
if (missing.length > 0) {
    const message = `[@cw-suite/api-events] Missing package metadata: ${missing.join(', ')}. Please update package.json.`;
